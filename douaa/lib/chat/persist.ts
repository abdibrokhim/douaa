...
// Database instance
let dbPromise: Promise<IDBPDatabase<DouaaDB>> | null = null;

// Function to get and initialize the database
function getDB(): Promise<IDBPDatabase<DouaaDB>> {
  if (!isBrowser) {
    return Promise.reject(new Error('IndexedDB is not available outside the browser'));
  }

  if (!dbPromise) {
    dbPromise = openDB<DouaaDB>(DB_NAME, DB_VERSION, {
      upgrade(db: IDBPDatabase<DouaaDB>, oldVersion: number, newVersion: number | null) {
